...
function orderAlphabetically(str) {
  const sortArr = str.split('').sort((a, b) => {
    if (a < b) return -1;
    return 1;
  });
  return sortArr.join('');
...
function isPalindrome(str) {
  ...
  let newStr2 = '';
  for (let ii = 0, j = newStr.length - 1; ii < newStr.length; ii += 1, j -= 1)
    newStr2 += newStr[j];
...
function reverseWords(str) {
  ...
  arr.forEach((item) => {
    let reverseItem = '';
    for (let i = 0, j = item.length - 1; i < item.length; i += 1, j -= 1)
      reverseItem += item[j];
    newArr.push(reverseItem);
  });
  return newArr.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].toLowerCase() === str[i].toUpperCase()) {
      ...
      alphabetIndex = str.charCodeAt(i) - alphabet.charCodeAt(0);
      rot13Index = (alphabetIndex + 13) % 26;
      newChar = alphabet[rot13Index];
    } else {
      alphabetIndex = str.charCodeAt(i) - ALPHABET.charCodeAt(0);
      rot13Index = (alphabetIndex + 13) % 26;
      newChar = ALPHABET[rot13Index];
